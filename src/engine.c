    ...
    if (!g->typing) {
        if (glfwGetKey(g->window, CRAFT_KEY_JUMP)) {
            if (g->flying) {
                ...
                dy = 14.0f * sqrtf(g->gravity_scale);
            }
        }
    }
...
int engine_run(int argc, char **argv) {
    ...
    // Sky shader: enhanced with a lightweight procedural aurora + star twinkle.
    // (See shaders/sky_fragment_aurora.glsl)
    program = load_program(
        "shaders/sky_vertex.glsl", "shaders/sky_fragment_aurora.glsl");
